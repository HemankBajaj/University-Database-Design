...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        WHERE dept_id = NEW.dept_id  AND entry_year = NEW.entry_year
    ) THEN
        RAISE EXCEPTION 'invalid';
    END IF;

    SELECT seq_number INTO dept_seq_number 
    FROM valid_entry 
    WHERE dept_id = new_dept_id 
    AND entry_year = new_entry_year;
    ...
    IF email_domain <> student_id_dept_id || '.iitd.ac.in' OR new_student_id <> NEW.student_id
        RAISE EXCEPTION 'invalid';
    END IF;
...
BEGIN
    ...
    entry_year := LEFT(NEW.student_id, 4)::INTEGER;
...
SELECT
    ...
    sc.session,
    sc.semester,
    COUNT(unique sc.student_id) AS number_of_students,
...
INSTEAD OF INSERT OR UPDATE ON student_courses
FOR EACH STATEMENT
...
BEGIN
    ...
    IF NEW.credits = 5 AND student_first_year <> session_year THEN
        RAISE EXCEPTION 'invalid';
    END IF;
...
BEGIN
    IF TG_OP = 'INSERT' THEN
        ...
        UPDATE student
        SET tot_credits = tot_credits + NEW.credits
        WHERE student_id = NEW.student_id;

    ELSIF TG_OP = 'UPDATE' THEN
        UPDATE student
        SET tot_credits = tot_credits - OLD.credits + NEW.credits
        WHERE student_id = NEW.student_id;
...
-- 2.4.1
CREATE OR REPLACE FUNCTION update_department_courses()
RETURNS TRIGGER AS $$
BEGIN
    IF OLD.dept_id <> NEW.dept_id THEN
        -- Update course ids in course_offers table
        UPDATE course_offers
        SET course_id = NEW.dept_id || substring(course_id from 4)
        WHERE course_id LIKE OLD.dept_id || '___';

        -- Update course ids in courses table
        UPDATE courses
        SET course_id = NEW.dept_id || substring(course_id from 4)
        WHERE course_id LIKE OLD.dept_id || '___';

        -- Update course ids in student_courses table
        UPDATE student_courses
        SET course_id = NEW.dept_id || substring(course_id from 4)
        WHERE course_id LIKE OLD.dept_id || '___';

        -- Update course ids in professor table
        UPDATE professor
        SET dept_id = NEW.dept_id
        WHERE dept_id = OLD.dept_id;

        -- Update course ids in student table
        UPDATE student
        SET dept_id = NEW.dept_id
        WHERE dept_id = OLD.dept_id;
    END IF;
...
RETURNS TRIGGER AS $$
BEGIN
    -- Check if there are any students in the department
    IF EXISTS (SELECT 1 FROM student WHERE dept_id = OLD.dept_id) THEN
        RAISE EXCEPTION 'Department has students';
    ELSE
        -- Delete all courses in the department from course_offers table
        DELETE FROM course_offers WHERE course_id LIKE OLD.dept_id || '___';

        -- Delete all courses in the department from courses table
        DELETE FROM courses WHERE course_id LIKE OLD.dept_id || '___';

        -- Delete all professors in the department from professor table
...
-- update trigger
CREATE TRIGGER update_department_courses_trigger
BEFORE UPDATE ON department
FOR EACH ROW
EXECUTE FUNCTION update_department_courses();
